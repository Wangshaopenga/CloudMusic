...
// 引入 plugin 中的函数
import { millisecondToMinute, roll } from "@/plugin";
...
// 定义计算属性 commentPagination 用来保存评论的页码，用于监听
const commentPagination = computed({
	get() {
		return info.commentPagination;
	},
});
...
const getComments = async (flag) => {
	if (songInfo.value) {
		let res = await comment(songInfo.value.id, info.commentPagination * 20);
		// 更新评论的总页数
...
const next = () => {
	if (songUrl.value) {
		...
		if (info.circulationType === 1) {
			...
			// 获取当前歌曲在播放列表中的索引
			let index = songInfo.value.currentIndex || songInfo.value.index;
...
const random = () => {
	if (playlist.value.length > 1) {
		let random;
		let currentIndex = songInfo.value.currentIndex || songInfo.value.index;
		const randomFn = () => {
...
const activeRowClass = ({ row, rowIndex }) => {
	if (rowIndex === songInfo.value.currentIndex) {
		return "active";
	} else if (
		!songInfo.value.currentIndex &&
		rowIndex === songInfo.value.index
	) {
		return "active";
	}
};
...
const dblclickInPlaylist = (row) => {
	...
	if (row.currentIndex) {
		// 修改当前歌曲的 url 和播放信息
		store.commit("changeCurrentSongUrl", playlist.value[row.currentIndex].url);
		store.commit("changeCurrentSongInfo", playlist.value[row.currentIndex]);
	} else {
		// 修改当前歌曲的 url 和播放信息
		store.commit("changeCurrentSongUrl", playlist.value[row.index].url);
		store.commit("changeCurrentSongInfo", playlist.value[row.index]);
	}
};
